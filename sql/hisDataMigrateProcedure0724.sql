...
-- 插入自由导入历史表
    ...
    select
        ...
        t.create_by,
        t.create_time
    from free_import t,t_lc_task ta where t.org_casno = ta.case_no and t.org_id = ta.org_id and  t.import_batch_no = ta.import_batch_no and ta.task_status = 3 AND NOW() >= (SELECT date_sub(ta.close_date, INTERVAL - days DAY));
-- 删除自由导入表
    DELETE t from free_import t,t_lc_task ta where t.org_casno = ta.case_no and t.org_id = ta.org_id and  t.import_batch_no = ta.import_batch_no and ta.task_status = 3 AND NOW() >= (SELECT date_sub(ta.close_date, INTERVAL - days DAY));
-- 插入信息查找记录历史表
...
-- 插入机器人任务明细历史表
    ...
    from t_lc_robot_task t,t_lc_robot_task_pandect p where t.robot_tast_id = p.robot_task_id and (p.robot_task_status = 2 or p.robot_task_status = 50);
-- 删除机器人任务明细表
