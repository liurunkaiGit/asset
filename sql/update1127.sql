...
ALTER TABLE t_lp_monthly_target_daily MODIFY COLUMN `commission_batch` VARCHAR(200) NOT NULL COMMENT '委案批次';
alter TABLE t_lp_monthly_target_daily MODIFY COLUMN `allocation_of_seats` decimal(15,2) DEFAULT NULL COMMENT '分配席位';
alter TABLE t_lp_monthly_target_daily MODIFY COLUMN `number_of_commission` int(9) DEFAULT NULL COMMENT '月初委案件数';
alter TABLE t_lp_monthly_target_daily MODIFY COLUMN `commission_amount` decimal(15,2) DEFAULT NULL COMMENT '月初委案金额';

DROP INDEX ymp ON t_lp_monthly_target;
-- DROP INDEX ympt ON t_lp_monthly_target;
...
alter table t_lp_project_information_daily MODIFY COLUMN `cases_number` varchar(200) DEFAULT NULL COMMENT '人均委案量';
alter table t_lp_project_information_daily MODIFY COLUMN `per_household` varchar(200) DEFAULT NULL COMMENT '户均金额';


alter table t_lp_attendance MODIFY COLUMN `attendance_date` date DEFAULT NULL COMMENT '出勤日期';
alter table t_lp_attendance_daily MODIFY COLUMN `attendance_date` date DEFAULT NULL COMMENT '出勤日期';
