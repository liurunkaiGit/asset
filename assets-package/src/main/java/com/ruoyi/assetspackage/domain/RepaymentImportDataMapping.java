package com.ruoyi.assetspackage.domain;

import org.springframework.context.annotation.Scope;
import org.springframework.stereotype.Component;


/**
 * @author guozeqi
 * @create 2019-12-26
 */
@Component
@Scope("prototype")
public class RepaymentImportDataMapping {

    private String orgCasNo; //机构案件号
    private String jyqtfy;//交易其他费用
    private String jylx;//交易利息
    private String jybj;//交易本金
    private String jyznf;//交易滞纳费
    private String jyType;//交易类型
    private String jyje;//交易金额
    private String productType;//产品类型
    private String jjh;//借据号
    private String csr;//催收人
    private String csjd;//催收节点
    private String fprq;//分配日期
    private String areaCenter;//区域中心
    private String acceptCity;//受理城市
    private String hth;//合同号
    private String dqsybYj;//地区事业部(一级)
    private String dqsybEj;//地区事业部(二级)
    private String wbqs;//外包期数
    private String wbjb;//外包经办
    private String warq;//委案日期
    private String curName;//客户名称
    private String khjlxm;//客户经理姓名
    private String sjrq;//数据日期
    private String sfwbcs;//是否外包催收
    private String sfjq;//是否结清
    private String bywa;//本月委案
    private String ajhsrq;//案件回收日期
    private String xfjrzh;//消费金融账号
    private String tzsx;//调整事项
    private String tzje;//调整金额
    private String zhzt;//账户状态
    private String hkrq;//还款日期
    private String hksyqqs;//还款时逾期期数
    private String hkje;//还款金额
    private String yqcplx;//逾期产品类型
    private String yqjd;//逾期阶段
    private String quotaProduct;//额度产品

    private String headRowNum; //表头行
    private String dataRowNum; //数据起始行


    public String getOrgCasNo() {
        return orgCasNo;
    }

    public void setOrgCasNo(String orgCasNo) {
        this.orgCasNo = orgCasNo;
    }

    public String getJyqtfy() {
        return jyqtfy;
    }

    public void setJyqtfy(String jyqtfy) {
        this.jyqtfy = jyqtfy;
    }

    public String getJylx() {
        return jylx;
    }

    public void setJylx(String jylx) {
        this.jylx = jylx;
    }

    public String getJybj() {
        return jybj;
    }

    public void setJybj(String jybj) {
        this.jybj = jybj;
    }

    public String getJyznf() {
        return jyznf;
    }

    public void setJyznf(String jyznf) {
        this.jyznf = jyznf;
    }

    public String getJyType() {
        return jyType;
    }

    public void setJyType(String jyType) {
        this.jyType = jyType;
    }

    public String getJyje() {
        return jyje;
    }

    public void setJyje(String jyje) {
        this.jyje = jyje;
    }

    public String getProductType() {
        return productType;
    }

    public void setProductType(String productType) {
        this.productType = productType;
    }

    public String getJjh() {
        return jjh;
    }

    public void setJjh(String jjh) {
        this.jjh = jjh;
    }

    public String getCsr() {
        return csr;
    }

    public void setCsr(String csr) {
        this.csr = csr;
    }

    public String getCsjd() {
        return csjd;
    }

    public void setCsjd(String csjd) {
        this.csjd = csjd;
    }

    public String getFprq() {
        return fprq;
    }

    public void setFprq(String fprq) {
        this.fprq = fprq;
    }

    public String getAreaCenter() {
        return areaCenter;
    }

    public void setAreaCenter(String areaCenter) {
        this.areaCenter = areaCenter;
    }

    public String getAcceptCity() {
        return acceptCity;
    }

    public void setAcceptCity(String acceptCity) {
        this.acceptCity = acceptCity;
    }

    public String getHth() {
        return hth;
    }

    public void setHth(String hth) {
        this.hth = hth;
    }

    public String getDqsybYj() {
        return dqsybYj;
    }

    public void setDqsybYj(String dqsybYj) {
        this.dqsybYj = dqsybYj;
    }

    public String getDqsybEj() {
        return dqsybEj;
    }

    public void setDqsybEj(String dqsybEj) {
        this.dqsybEj = dqsybEj;
    }

    public String getWbqs() {
        return wbqs;
    }

    public void setWbqs(String wbqs) {
        this.wbqs = wbqs;
    }

    public String getWbjb() {
        return wbjb;
    }

    public void setWbjb(String wbjb) {
        this.wbjb = wbjb;
    }

    public String getWarq() {
        return warq;
    }

    public void setWarq(String warq) {
        this.warq = warq;
    }

    public String getCurName() {
        return curName;
    }

    public void setCurName(String curName) {
        this.curName = curName;
    }

    public String getKhjlxm() {
        return khjlxm;
    }

    public void setKhjlxm(String khjlxm) {
        this.khjlxm = khjlxm;
    }

    public String getSjrq() {
        return sjrq;
    }

    public void setSjrq(String sjrq) {
        this.sjrq = sjrq;
    }

    public String getSfwbcs() {
        return sfwbcs;
    }

    public void setSfwbcs(String sfwbcs) {
        this.sfwbcs = sfwbcs;
    }

    public String getSfjq() {
        return sfjq;
    }

    public void setSfjq(String sfjq) {
        this.sfjq = sfjq;
    }

    public String getBywa() {
        return bywa;
    }

    public void setBywa(String bywa) {
        this.bywa = bywa;
    }

    public String getAjhsrq() {
        return ajhsrq;
    }

    public void setAjhsrq(String ajhsrq) {
        this.ajhsrq = ajhsrq;
    }

    public String getXfjrzh() {
        return xfjrzh;
    }

    public void setXfjrzh(String xfjrzh) {
        this.xfjrzh = xfjrzh;
    }

    public String getTzsx() {
        return tzsx;
    }

    public void setTzsx(String tzsx) {
        this.tzsx = tzsx;
    }

    public String getTzje() {
        return tzje;
    }

    public void setTzje(String tzje) {
        this.tzje = tzje;
    }

    public String getZhzt() {
        return zhzt;
    }

    public void setZhzt(String zhzt) {
        this.zhzt = zhzt;
    }

    public String getHkrq() {
        return hkrq;
    }

    public void setHkrq(String hkrq) {
        this.hkrq = hkrq;
    }

    public String getHksyqqs() {
        return hksyqqs;
    }

    public void setHksyqqs(String hksyqqs) {
        this.hksyqqs = hksyqqs;
    }

    public String getHkje() {
        return hkje;
    }

    public void setHkje(String hkje) {
        this.hkje = hkje;
    }

    public String getYqcplx() {
        return yqcplx;
    }

    public void setYqcplx(String yqcplx) {
        this.yqcplx = yqcplx;
    }

    public String getYqjd() {
        return yqjd;
    }

    public void setYqjd(String yqjd) {
        this.yqjd = yqjd;
    }

    public String getQuotaProduct() {
        return quotaProduct;
    }

    public void setQuotaProduct(String quotaProduct) {
        this.quotaProduct = quotaProduct;
    }

    public String getHeadRowNum() {
        return headRowNum;
    }

    public void setHeadRowNum(String headRowNum) {
        this.headRowNum = headRowNum;
    }

    public String getDataRowNum() {
        return dataRowNum;
    }

    public void setDataRowNum(String dataRowNum) {
        this.dataRowNum = dataRowNum;
    }




}
