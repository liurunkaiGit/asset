...
public class DownLoadFromHttpsUtil implements X509TrustManager {
    ...
    public static void downLoadFromHttps2(String url,String savePath,String fileName) {
        log.info("开始下载，下载地址{}", url);
        if (!(url == null || StringUtils.equals("", url))) {
            try {
                byte[] bytes = downLoadFromUrlHttps2(url);
                FileOutputStream fileOut = new FileOutputStream(savePath+File.separator+fileName);
    ...
    public static byte[] downLoadFromUrlHttps2( String urlStr) throws Exception {
        // 创建SSLContext
        SSLContext sslContext = SSLContext.getInstance("SSL");
        TrustManager[] tm = {new DownLoadFromHttpsUtil()};
        // 初始化
