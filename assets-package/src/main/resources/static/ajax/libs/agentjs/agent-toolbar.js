var AgentBar = function(option) {
	...
	this.makeCall = function(destType, destDesc, callerId, tenantId, dialPrefix) {
		return client.makeCall(destType, destDesc, callerId, tenantId, dialPrefix);
	};
	...
	this.consultCall = function(destType, destDesc, callerId, actionType) {
		return client.consultCall(destType, destDesc, callerId, actionType);
	};
	...
	this.startMute = function() {
		return client.startMute();
	};

	// 结束录音
	this.stopMute = function() {
		return client.stopMute();
	};

	// 设置SIPHead
	this.setSipHead = function(headKey, headValue) {
		return client.setSipHead(headKey, headValue);
	};

	// 获取SIPHead
	this.getSipHead = function(headKey, success, error) {
		return client.getSipHead(headKey, success, error);
	};

	// 坐席媒体控制
	this.execCommand = function(appName, appData, privateData) {
		return client.execCommand(appName, appData, privateData);
	};

	client.onOnline= function(event) {

		var owReasons = client.getOption('owReasons');
		...
		if (owReasons) {
			...
			if (owHtml) {
				container.find('.otherwork').parent().remove();
				container.find('#btnLogout').parent().before(owHtml);
			}
		}
	...
	client.onLogin= function(event) {
		if (isFunction(self.onLogin)) {
			self.onLogin(event);
		}
	};
	
	client.onLogout= function(event) {
	...
	client.onIMOffering = function(event) {

		if (isFunction(self.onIMOffering)) {
			self.onIMOffering(event);
		}
	};

	client.onIMConnected = function(event) {
		
		if (isFunction(self.onIMConnected)) {
			self.onIMConnected(event);
		}
	};

	client.onIMReleased = function(event) {
		if (isFunction(self.onIMReleased)) {
			self.onIMReleased(event);
		}
	};
	...
	function onStatusChanged(event) {
		...
		if (status) {
			...
			// 根据签入状态时间校准时间偏差，用于计算状态时长
			if (event.eventType == 1 && event.currStatus == AST_LOGIN) {
	...
	var agentbarTemplate = '<div class="agentbar">'
		...
		+ '			<input type="text" class="form-control" data-toggle="dial" id="txtNumber" maxlength="256">'
