var AgentClient = function(option) {
	...
	function makeCall(destType, destDesc, callerId, tenantId, dialPrefix) {
		...
		return sendCommand(createMakeCallCommand(agent.agentId, destType, destDesc, callerId, tenantId, dialPrefix));
	}
	...
	function consultCall(destType, destDesc, callerId, actionType) {
		...
		if (agent.isLogin) {
			...
			return sendCommand(createConsultCallCommand(agent.agentId, agent.agentDn, destType, destDesc, ACTION_TYPE_TRANSFER, callerId, actionType));
		}
	...
	function getCallData(dataKey, success, error) {
		...
		if (agent.isLogin && dataKey) {
			...
			return sendCommand(createGetCallDataCommand(agent.agentId, agent.agentDn, dataKey));
		}
	...
	function getAgentState(agentId, success, error) {
		...
		if (agent.isLogin) {
			...
			var timeout = true;
			var token = PubSub.subscribe(Resp_IContact_GetAgentState, function(topic, response) {
            ...
            return sendCommand(createGetAgentStateCommand(agent.agentId, agent.agentDn, agentId));
        }
	...
	function createMakeCallCommand(agentId, destType, destDesc, callingParty, tenantId, dialPrefix) {
		return {
			...
			Key_IContact_DialPrefix :(dialPrefix || '') + ''
		};
	}
	...
	function createConsultCallCommand(agentId, agentDn, destType, destDesc, consultType, callingParty, actionType) {
		return {
			...
			Key_IContact_CallingParty : callingParty + '',
			Key_IContact_ActionType : parseInt(actionType || 0)
		};
	}
	...
	function fireAgentStatusChangedEvent(event) {
		...
		if (isFunction(self.onStatusChanged)) {
			self.onStatusChanged({
				...
				agentId : event.Key_IContact_AgentID,
	...
	// 外呼
	this.makeCall = function(destType, destDesc, callerId, tenantId, dialPrefix) {
		return makeCall(destType, destDesc, callerId, tenantId, dialPrefix);
	};
	...
	// 咨询
	this.consultCall = function(destType, destDesc, callerId, actionType) {
		return consultCall(destType, destDesc, callerId, actionType);
	};
