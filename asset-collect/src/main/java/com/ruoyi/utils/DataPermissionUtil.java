...
public class DataPermissionUtil {
    ...
    public Set<Long> findDeptDataPermissionByUser(SysUser sysUser) {
        ...
        if (dataScopeSet.contains(DataScopeAspect.DATA_SCOPE_CUSTOM)) {
            ...
            sysRoles.stream()
                    .forEach(sysRole -> {
                        List<SysRoleDept> roleDepts = this.tLcTaskMapper.selectDeptIdByRoleId(sysRole.getRoleId());
    ...
    private void getChildDept(Set<Long> deptSet, Long deptId) {
        List<SysDept> sysDeptList = this.tLcTaskMapper.selectDeptByParentId(deptId);
        sysDeptList.stream()
                .forEach(sysDept -> {
