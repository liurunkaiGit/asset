  ...
  /*  @Autowired
    ...
    public Set<Long> findDeptDataPermissionByUser(SysUser sysUser) {
        ...
        if (dataScopeSet.contains(DataScopeAspect.DATA_SCOPE_CUSTOM)) {
            ...
            sysRoles.stream()
                    .forEach(sysRole -> {
                        List<SysRoleDept> roleDepts = this.sysDeptMapper.selectDeptIdByRoleId2(sysRole.getRoleId());
    ...
    private void getChildDept(Set<Long> deptSet, Long deptId) {
        List<SysDept> sysDeptList = this.sysDeptMapper.selectDeptByParentId2(deptId);
        sysDeptList.stream()
                .forEach(sysDept -> {
     ...
     * @return
     */
    public Set<Long> selectDataPerNew(SysUser sysUser) {
        Set<Long> deptIds = null;
        boolean allDataPermissionByUser = findAllDataPermissionByUser(sysUser);
        if (!allDataPermissionByUser) {
            Set<Long> deptDataPermission = findDeptDataPermissionByUser(sysUser);
            if (deptDataPermission != null && deptDataPermission.size() > 0) {
                deptIds = deptDataPermission;
            } else {
                boolean selfDataPermission = findSelfDataPermissionByUser(sysUser);
                if (!selfDataPermission) {
                    log.error("没有查看数据的权限，请联系管理员授权");
                    throw new RuntimeException("没有查看数据的权限，请联系管理员授权");
                }
